...
const Assistant = () => {
  ...
  return (
    <>
      <article
        className={`assistant-help ${openChatbot}`}
      ...
      <article className={`chat-bot-content ${displayToggle}`}>
        <article className='header-chat-bot'>
          <h4>Powered by Nezzed</h4>
          <input
            type='button'
            onClick={handleClickClose}
            value='x'
            className='btn btn-danger close-btn mt-4'
            title='Cerrar'
          />
        </article>
        <article
          ref={chatbotScroll}
          className={`body-chat-bot ${lessBodyChatbot}`}>
          {arrayMsj.map((obj, i) => {
            let classVerify;
            ...
            return (
              <article
                key={i}
                className={`${classVerify} col-12 col-sm-12 col-lg-12 col-xl-12`}>
                <p>{obj.msj}</p>
              </article>
            );
          })}
        ...
        <article className='input-chatbot'>
          <form onSubmit={handleSubmit}>
            <input
              type='text'
              onChange={handleChangeMsj}
              value={msj}
              placeholder='Escriba una consulta'
            />
            <ul className='list-tools-chat-bot'>
              <li>
                <i className='large material-icons'>keyboard_voice</i>
              </li>
              <li onClick={handleClickOpenEmoji}>
                <i className='btn-emoji large material-icons'>{emojiToggle}</i>
              </li>
            </ul>
          </form>
        </article>
        <article className={`content-emojis-chat-bot ${listEmojis}`}>
          <ul>
            {emojis.map((obj, i) => (
              <li key={i} onClick={handleSelectEmoji} title={obj}>
                {obj}
              </li>
            ))}
          </ul>
